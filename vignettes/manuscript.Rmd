...
We measured fluxes for three complete years from the alluvium wetland, four complete years from the old peat wetland, and six complete years from the young peat wetland. Old peat wetland fluxes from 2013 were omitted from the annual flux intercomparison because the water table dropped below the land surface for 24 days, and fluxes from this year were thus not representative of stable state restored wetland fluxes. Annual *NEE* budgets were not significantly different across the three wetland sites (ANOVA, *P* = 0.7), though all sites were CO~2~ sinks on average (Fig. 4a). Mean annual *NEE* was `r mean(alluvium_yearly$tNEE)` $\pm$ `r sd(alluvium_yearly$tNEE)`, `r mean(peat19_s$tNEE)` $\pm$ `r sd(peat19_s$tNEE)`, and `r mean(peat6_yearly$tNEE)` $\pm$ `r sd(peat6_yearly$tNEE)` g CO~2~-C m^-2^ yr^-1^ for the alluvium, old peat, and young peat wetlands, respectively (mean $\pm$ SD). Similarly, annual *GEP* (ANOVA, *P* = 0.97) and *ER* (ANOVA, *P* = 0.9) did not vary across the three wetland sites (Fig. 4b,c). Mean annual CH~4~ fluxes varied significantly across sites (ANOVA, *P* < 0.01), and annual CH~4~ fluxes from the alluvium wetland were significantly lower than those from either peat site (Tukey's HSD, *P* < 0.05) (Fig. 4d). Mean annual CH~4~ fluxes did not vary between young and old peat wetlands (Tukey's HSD, *P* = 0.92) (Fig. 4d). Annual CH~4~ fluxes were `r mean(alluvium_yearly$tCH4)` $\pm$ `r sd(alluvium_yearly$tCH4)`, `r mean(peat19_s$tCH4)` $\pm$ `r sd(peat19_s$tCH4)`, and `r mean(peat6_yearly$tCH4)` $\pm$ `r sd(peat6_yearly$tCH4)` g CH~4~-C m^-2^ yr^-1^ for the alluvium, old peat, and young peat wetlands, respectively (mean $\pm$ SD). Consequently, annual GHG equivalent budgets were higher from the old peat (`r mean(peat19_s$GHGbalance)` $\pm$ `r sd(peat19_s$GHGbalance)`) and young peat sites (`r mean(peat6_yearly$GHGbalance)` $\pm$ `r sd(peat6_yearly$GHGbalance)` g CO~2~eq. m^-2^ yr^-1^) compared to the alluvium site (`r mean(alluvium_yearly$GHGbalance)` $\pm$ `r sd(alluvium_yearly$GHGbalance)`); however, these differences were not statistically significant.

```{r fig4, fig.cap="Annual fluxes of (a) net ecosystem exchange (*NEE*), (b) ecosystem respiration (*ER*), (c) gross ecosystem photosynthesis (*GEP*), and (b) methane flux (*F~CH4~*. Letters denote significant differences. Solid horizontal lines are medians, boxes are the interquartile range (IQR), and whiskers are the 95% confidence interval  ", echo=FALSE}
...
## CO~2~ and CH~4~ Flux Times Series
