...
The three wetland sites on Sherman and Twitchell Islands experienced a similar conditions for major meteorologic and hydrologic drivers of ecosystem CO~2~ and CH~4~ fluxes (Fig. 2). Incoming *PAR* and *T~a~* exhibited strong seasonality, where peak values were observed in the summer months, and daily mean values were similar across all three wetland sites (Fig. 2a,b). Water temperature exhibited a similar seasonality across all three sites; however, *T~w~* was lower at the old peat compared to the two young wetland sites (Fig. 2c). Wetlands were similarly managed to maintain year-round inundation, where wetland ~WTD~ was meant to be above the land surface across all years (Fig. 2d). In some cases wetland water control structures would fail causing periods where the ~WTD~ would drop below the land surface. These events were common in summer 2013 at the old peat wetland, where the ~WTD~ dropped below the land surface many times over the course of the summer (Fig. 2d). A more detailed summary of annual meteorologic conditions across sites and years can be found in Eichelmann et al. (in review).

```{r fig2, fig.cap="Daily mean (a) photosynthetically active radiation (*PAR*), (b) air temperature (*T~a~*), (c) water temperature (*T~w~*), and (d) water table depth (*WTD*) for the alluvium, young peat, and old peat wetland sites. The dashed line on panel d represents the ground surface elevation at the water table well location, and the sold line represents daily mean *WTD*.", warning=FALSE, fig.height=7, fig.width=8}
all_daily$date <- as.Date(all_daily$datetime) #create a date variable for x variable
...
d <- ggplot(all_daily, aes(x=date, y=WTD, color=site)) +
  geom_line(size=0.5) + theme_bw() + geom_hline(yintercept=0) +
  scale_x_date(date_breaks = "1 year", labels = date_format("%Y-%m")) +
  ylab("WTD (cm)") +
...
#compute signficant differences among sites. These output are reported in the following section, and significance is reported at 0.05, 0.001, and 0.0001 levels for P < 0.05. The maximum P value is used when multiple comparisons are cited
...
```
HCl-extractable Fe concentrations were significantly higher in alluvium compared to peat sites in both recent (Tukey's HSD, *P* < 0.0001) and parent soil horizons (Fig. 3a; Tukey's HSD, *P* < 0.001). At the alluvium site, HCl-extractable Fe concentrations did not vary with depth (Tukey's HSD, *P* = 0.6); however at the peat sites, HCl-extractable Fe concentrations were lower in both young peat (Tukey's HSD, *P* = 0.051) and old peat sites (Tukey's HSD, *P* < 0.001). Mean Fe concentrations were `r mean(soils_df$FeT_mg.g[which(soils_df$site == "Young Peat" & soils_df$horizon == "Parent")]) - mean(soils_df$FeT_mg.g[which(soils_df$site == "Young Peat" & soils_df$horizon == "Recent")])` and `r mean(soils_df$FeT_mg.g[which(soils_df$site == "Old Peat" & soils_df$horizon == "Parent")]) - mean(soils_df$FeT_mg.g[which(soils_df$site == "Old Peat" & soils_df$horizon == "Recent")])` mg Fe g^-1^ lower in recent horizon for the young and old peat sites, respectively. Poorly crystalline Fe concentrations were roughly two orders of magnitude lower than HCl-extractable Fe concentrations across all three sites (Fig. 3a,b), and trends in poorly crystalline Fe were similar, as concentrations were signficantly higher in alluvium horizons compared to either peat wetland horizons (Tukey's HSD, *P* < 0.0001). Poorly crystalline Fe concentrations trended lower in recent horizons across all sites, but none of these differences were significant (Tukey's HSD, *P* > 0.08). Mean poorly crystalline Fe concentrations were over an order of magnitude higher in alluvium soils (`r mean(soils_df$FePC_mg.g[which(soils_df$site == "aalluvium")])` mg Fe g^-1^ soil) compared to the young (`r mean(soils_df$FePC_mg.g[which(soils_df$site == "Young Peat")])` mg Fe g^-1^ soil) and old (`r mean(soils_df$FePC_mg.g[which(soils_df$site == "Old Peat")])` mg Fe g^-1^ soil) peat sites.

```{r fig3, fig.cap="Soil (a) HCl-extractable and (b) poorly crystalline iron (Fe), as well as (c) percent soil carbon (%C) in recent and parent sediments across three wetland sites (*n* 14 per depth). Solid horizontal lines are medians, boxes are the interquartile range (IQR), and whiskers are IQR. Points above each boxplot are the measured soils data for each site.", fig.width=8, fig.height=3, warning=FALSE}
source("R/soils.R") #process soils data into summary dataframe
