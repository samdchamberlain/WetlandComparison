---
title: Soil properties and sediment accretion modulate methane fluxes from restored
  wetlands
...
a <- ggplot(new_wetlands, aes(x=site, y=FeT_mg.g, fill=horizon)) +
  ...
  annotate("text",x=0.6,y=32,label="(a)", fontface = "bold") +
...
b <- ggplot(new_wetlands, aes(x=site, y=FeII_mg.g, fill=horizon)) +
  ...
  annotate("text", x=0.6, y=25, label="(b)", fontface = "bold") +
  boxplot_theme + theme(legend.position="none")
...
c <- ggplot(new_wetlands, aes(x=site, y=FePC_mg.g, fill=horizon)) +
  ...
  annotate("text", x=0.6, y=0.5, label="(c)", fontface = "bold") +
  boxplot_theme + theme(legend.position="none")
...
d <- ggplot(new_wetlands, aes(x=site, y=pH, fill=horizon)) +
  ...
  ylab(expression("pH")) +
  annotate("text", x=0.6, y=7.6, label="(d)", fontface = "bold") +
  boxplot_theme +
...
e <- ggplot(new_wetlands, aes(x=site, y=C, fill=horizon)) +
  ...
  annotate("text", x=0.6, y=30, label="(e)", fontface = "bold") +
  boxplot_theme + theme(legend.position="none")
...
f <- ggplot(new_wetlands, aes(x=site, y=N, fill=horizon)) +
  ...
  ylab(expression("%N")) +
  #scale_y_continuous(limits=c(10,30)) +
  annotate("text", x=0.6, y=1.6, label="(f)", fontface = "bold") +
  boxplot_theme + theme(legend.position="none")
...
a <- ggplot(trajectory, aes(x=dday/365, y=mgCH4, color=site)) +
  ...
  annotate("text", x=5, y=Inf, label="(a)", vjust=1, hjust=1, fontface = "bold") +
...
b <- ggplot(trajectory, aes(x=dday/365, y=gCO2, color=site)) +
  ...
  annotate("text", x=5, y=Inf, label="(b)", vjust=1, hjust=1, fontface = "bold") +
...
f <- ggplot(trajectory, aes(x=dday/365, y=WTD, color=site)) +
  ...
  annotate("text", x=5, y=Inf, label="(f)", vjust=1, hjust=1, fontface = "bold") +
  theme_bw() + 
...
set.seed(1000)
gam_fit <- train(mgCH4 ~ PAR + Tair + mVPD + PA + u. + mET + gER + gGEP + mGCC, data = peat19_daily, method = "gam",
                  preProcess = c("knnImpute"),
                  trControl = trainControl(method = "cv"),
                  na.action = na.pass)

# extract prediction datasets, only using time periods when wetlands were filly vegetated
...
# generate predictions for peat and alluvium wetlands
alluvium$pred <- predict.train(gam_fit, alluvium, na.action = na.pass)
peat$pred <- predict.train(gam_fit, peat, na.action = na.pass)

# ...and residuals
alluvium$resid <- alluvium$mgCH4 - alluvium$pred
peat$resid <- peat$mgCH4 - peat$pred
...
The trained GAM performed relatively well against observed peat wetland CH~4~ fluxes (*r^2^* = `r  summary(regr_peat)$r.squared`). Within each year (2012 - 2017), residual CH~4~ fluxes were distributed around zero and skewed slightly negative, indicating some overestimation in predicted fluxes. The model residual interquartile range (IQR) always overlapped with zero, except for 2017 when multiple disturbance events reduced CO2 and CH4 fluxes from the peat wetland (Fig. 5).
...
a <- ggplot(peat, aes(x=factor(year), y=resid)) +
  ...
  xlab("Year") + ylab(expression("Residual "~F[CH4]~" (mg C"~m^{-2}~d^{-1}~")")) +
  ggtitle("Peat wetland") +
...
a <- ggplot(all_pst14, aes(x=year, y=tCH4, color=site)) +
        ...
        annotate("text", x=2017, y=60, label="(a)", fontface = "bold") +
        time_series + theme(legend.title=element_blank(), legend.margin = margin(0, 0, 0, 0),
                            ...
                            legend.text = element_text(size=8), legend.background = element_rect(fill = "transparent"),
                            axis.title.x = element_blank())
...
b <- ggplot(all_pst14, aes(x=year, y=GHGbalance, color=site)) +
        ...
        annotate("text", x=2017, y=3000, label="(b)", fontface = "bold") +
