...
```{r Fig5, echo=FALSE, warning=FALSE, message=FALSE, fig.height=3, fig.width=10, fig.align = "center", fig.cap="Fig. 5: Soil pH and iron in surface and deep wetland sediments"}
source("R/soils.R") #process soils data into summary dataframe

a <- ggplot(soils_df, aes(x=site, y=FeT_mg.g, fill=horizon)) +
  geom_boxplot() +
  ylab(expression("Total Fe (mg "~g^{-1}~"soil)"))
...
soil_properties <- soils_df %>%
  ...
  summarise(pH_m = mean(pH),
            pH_sd = sd(pH),
            Fe_all_m = mean(FeT_mg.g),
            Fe_all_sd = sd(FeT_mg.g),
...
```{r stat_soils}
fe_aov <- aov(FeT_mg.g ~ site*horizon, data=soils_df); summary(fe_aov); 
TukeyHSD(fe_aov)

pc_aov <- aov(FePC_mg.g ~ site*horizon, data=soils_df); summary(pc_aov); 
TukeyHSD(pc_aov)
```
...
```{r diel figure, fig.height = 8, fig.width = 8, fig.align = "center",fig.cap="Fig. 4: Diel patterns by site and year", echo=FALSE}
...
```{r cost_analysis}
# Use pre-restoration values from Knox et al. 2015 for corn and pasture GHG balance
site <- c("Ag Sites", "Ag Sites", "Ag Sites")
GHGbalance <- c(1444, 1631, 2094)
ag_sites <- data.frame(site, GHGbalance)

#bind datasets
cost_df <- rbind(cost_df, ag_sites)
cost_df$cost <- cost_df$GHGbalance*(10000)*(10^-6)

#plot average costs
ggplot(cost_df, aes(x=site, y=cost)) +
  geom_boxplot()

test <- aov(cost_df$cost ~ cost_df$site); summary(test); TukeyHSD(test)
```
